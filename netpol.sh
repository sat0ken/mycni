...
kubectl get networkpolicies.networking.k8s.io -A -o json | jq -c .items[] | while read -r item; do
	...
	# ipsetのポート指定がうまく動かない？
	# ingress_ports=$(echo "$item" | jq -r '.spec.ingress[].ports[]? | select(.protocol == "TCP") | .port')
	# if [ -n "$ingress_ports" ]; then
	# 	ipset_type="hash:ip,port"
	# 	# ingressのportsがセットされていたら<ターゲットのPodのIPアドレス,port>に更新する
	# 	for ((i = 0; i < ${#pod_ips[@]}; i++)); do
	# 		pod_ips[$i]="${pod_ips[$i]},$ingress_ports"
	# 	done
	# else
	# 	ipset_type="hash:ip"
	# fi
